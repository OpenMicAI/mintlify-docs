...
description: 'Complete reference for the static_params object when creating or updating a transfer_call tool via the API, including cold transfers, warm transfers, and webhook mode.'
slug: api-reference/tools/transfer-call-static-params
---

When creating a tool with `type: "transfer_call"` via the [Create Tool](/api-reference/tools/create-tool) endpoint, the `static_params` object configures how calls are transferred — including routing, transfer type, and warm transfer behavior.
